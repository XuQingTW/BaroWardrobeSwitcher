...
Version 0.5.0 is verified for Barotrauma 1.13.4.0 after completing its source/API checks and in-game Windows/Linux release matrix. See [ARCHITECTURE.md](ARCHITECTURE.md) for component boundaries and [COMPATIBILITY.md](COMPATIBILITY.md) for the pinned game/LuaCs contracts and release gates.
...
5. If the look was active before a scene or character change, it is rebuilt for the new character instance and reapplied after Barotrauma's initial equipment burst settles. A saved-but-inactive or manually cleared look stays inactive.
...
- A successful C# load prints:
  - `[Baro Wardrobe Switcher] C# visual override v0.5.0 initializing.`
...
- Client saved looks use schema-v2 `ClientLook.json` under the user's Barotrauma local app data. The loader atomically migrates old `PersistentClientLook.txt` and v1 JSON data, quarantines corrupt files, and retains a `.v1.bak` during the v0.5.0 compatibility window. Saved looks remain portable across campaigns and servers.
...
- Server persistence uses schema-v2 `ServerLooks.json` and stable `Client.AccountId` representations. Anonymous clients can sync during the current server session but are never persisted by display name.
...
- Protocol 2 adds hello negotiation, operation IDs, revisions, acknowledgements, idempotent retry, and stale-command rejection. The original six message names remain as a v1 bridge in v0.5.0 and are scheduled for removal in v0.6.0.
