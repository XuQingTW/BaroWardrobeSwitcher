...
- Multiplayer uses a small server-side Lua sync helper. Apply requests include the client's saved visual identifiers, so a look can be rebuilt after scene/campaign changes even if the server only had stale character item IDs. The server persists saved wardrobe item identifiers by stable client key, performs the server-authoritative removal, and broadcasts apply/clear events so other clients with LuaCs and C# scripting enabled can see the active look.
- Server persistence only uses stable SteamID or account identifiers. Clients without a stable nonzero identifier get a temporary in-memory multiplayer key so saved looks survive scene changes during the same server session, but their server-side wardrobe state is not persisted or restored across reconnect/server-restart boundaries.
