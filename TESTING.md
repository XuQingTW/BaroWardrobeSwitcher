...
- Save clothing that leaves the hands or feet exposed, apply it, then equip/swap/remove clothing with gloves or boots in the same saved slot. The saved fashion must retain its own limb visibility with `Hide Empty-Slot Equipment` both on and off. Also test fashion that deliberately hides a limb; its mask must remain intact. The renderer mask probe covers both mask values for left/right hands, legs, and feet, plus visible empty-slot suits, forced diving masks, explicit hair visibility, equipment removal, and cleanup.
- On page two, `Hide Empty-Slot Equipment` starts `On`. Save a look with clothing but no bag, apply it, and equip a backpack: it stays hidden while on and appears immediately when switched off, even after the fashion clothing has drawn on the same torso. Switch back on to hide it again. Repeat with a fully empty look, a saved bag (real bags must stay overridden in either mode), multiple active crew looks, a multiplayer observer, and the XDS-01 engine appendages. Equip/swap/remove real items while off, clear/reapply the look, and restart to verify the local choice persists without altering saved outfits or gameplay. In pressure, custom diving mode must use its own saved empty slots, while `Diving suit only` must hide other equipment with either toggle value. When a visible diving suit hides other wearables, verify wardrobe injected arm, leg, and foot sprites are also suppressed in both normal and fallback fashion draw paths. At minimum UI scale the enlarged second page should fit without a scrollbar; hover the new control to read its help.
...
- Movement-speed regression: restart the game after updating (the old animation hook can leave a per-character failed-animation cache in the running game). In single-player, measure walking, running, and backward movement over the same dry corridor with a real diving suit and no active look. Apply ordinary fashion and enable fashion movement: the pose remains cosmetic, but movement must not exceed the suit's corresponding native animation speed. Compare ordinary, combat, abyss, and modded suits; existing slower movement and equipment stat modifiers must remain effective. Toggle back to equipment movement repeatedly without re-equipping: native suit animations and speeds must resume. Remove/swap the suit, clear/reapply the look, switch between two crew members, and change rounds; no speed limit may remain from the old suit or another character. Swimming and crouching must remain unchanged. Repeat as multiplayer owner/observer. The automated movement probe checks forward/backward limits, type isolation, expiry, per-session isolation, suppression success, toggle recovery, and clear cleanup; it does not measure live physics or visual gait.
...
- Footstep lifecycle report: enable `Follow Fashion`, walk, dump diagnostics, then Clear/Apply without saving again, walk, and dump diagnostics again. Repeat after death/revival and a round transition in single-player and multiplayer. Record the fashion prefab and installed client/server versions. Compare `fashionFootsteps`, `footstepCalls`, `fashionFootstepCalls`, and `fashionFootstepTags` in `WardrobeClient.log`: the boolean records the renderer's actual policy, counters show whether the hook runs and completes substitution, and tags list the captured limb/sound pairs. No per-step file logging is performed. Automated adapter tests cover reuse, fresh capture, replacement characters, and current/legacy multiplayer clear/reapply; they do not verify audible playback or interactions with other mods.
